...
void linkedlist :: reverseLink(linkedlist mylist){
    if (head == NULL){
        cout<<"The list is empty "<<endl;
    }
    else{
...
//     struct node* chead = mylist.gethead();
//     cout<<"printing head value from reverse link function: "<<chead->data;
//     if (chead == nullptr || chead->next == nullptr){
//         return ;
//     }
//     struct node* prev = chead;
//     struct node* ptr = chead->next;
...
void linkedlist :: sethead(linkedlist::node* chead){
    this->head = chead;
}
